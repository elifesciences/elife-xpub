...
const ReviewerSuggestions = ({ values, setFieldValue }) => {
  ...
  const SuggestedReviewerFocusHandler = index => {
    ...
    const maximumReached = values.suggestedReviewers.length === MAX_REVIEWERS

    if (visibleReviwersHaveData() && !maximumReached) {
      // create a new reviewer entry row
      values.suggestedReviewers.push({ name: '', email: '' })
    }
  }
    ...
    <React.Fragment>
      ...
      <Box mb={5}>
        ...
        {values.suggestedReviewers.map((_, index) => (
          ...
          <Box key={index} mb={2}>
            <SuggestedReviewer
              focusHandler={SuggestedReviewerFocusHandler}
              index={index}
            />
          </Box>
        ))}
