            ...
            def actions = [
                "browser": {
                  ...
                  withCommitStatus({
                      ...
                      sh "IMAGE_TAG=${commit} NODE_ENV=production NODE_CONFIG_ENV=unit-test docker-compose -f docker-compose.yml -f docker-compose.ci.yml run --rm --name elife-xpub_app_test app bash -c 'yarn test:log-emails'"
                  }, 'test:browser', commit)
                },
                "api": {
