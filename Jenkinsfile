        ...
        stage 'Browser Tests', {
            try {
                ...
                withCommitStatus({
                    ...
                    sh "IMAGE_TAG=${commit} NODE_ENV=production NODE_CONFIG_ENV=unit-test docker-compose -f docker-compose.yml -f docker-compose.ci.yml run --rm --name elife-xpub_app_test app npm test packages/component-logger/ --testMatch=**/log.filter-ci.js"
                }, 'test:browser', commit)
            } finally {
