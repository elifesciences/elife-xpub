            ...
            def actions = [
                ...
                'test': {
                    withCommitStatus({
                        ...
                        sh "IMAGE_TAG=${commit} docker-compose -f docker-compose.yml -f docker-compose.ci.yml run -e CODECOV_TOKEN=\$(cat .codecov-token) -e XPUB_COMMIT=${commit} --rm --name elife-xpub_app_test app npm run test-ci"
                    }, 'test', commit)
                },
                'test:dependencies': {
                    withCommitStatus({
