    ...
    node('containers-jenkins-plugin') {
        ...
        stage 'Browser Tests', {
            try {
                ...
                sh "IMAGE_TAG=${commit} docker-compose -f docker-compose.yml -f docker-compose.ci.yml run --rm --name elife-xpub_wait_postgres app bash -c './scripts/wait-for-it.sh -t 180 localhost:3000'"
                withCommitStatus({
